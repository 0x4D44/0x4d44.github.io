...
**ID grammar.** `ALM-REQ-HOST-NNNNN`.
- **ALM** — this repo's acronym.
...
- Legacy `ALM-NNNNN` ids (pre-conversion) remain valid, verbatim, forever.
...
`ALM-REQ-<HOST>-*` files, take `max(NNNNN)+1`, and commit that one new
...
```markdown
# ALM-REQ-HOST-00001 — Short imperative requirement title
