...
pub const MAX_WORLD_CELLS: usize = 256 * 256;''',
    ...
    (
        ...
        let mut max_intervention_id = None;
        for intervention in &self.interventions {
            ...
            max_intervention_id = Some(
                max_intervention_id.map_or(intervention.id, |current: u64| current.max(intervention.id)),
            );
        }
...
# The first replacement above intentionally preserves an existing marker; skip it.
...
print("Checkpoint, UI evidence and scientific-language corrections are materialised.")
