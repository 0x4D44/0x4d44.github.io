...
(function (global) {
  ...
  var API_LEDGER = [
    ...
    { year: 1995, name: "NVLIB", who: "NVIDIA", family: "vendor",
      what: "NV1's own library, given away with the SDK. It drew quadratic patches, because that is what the chip drew. Sega's Saturn used quads too, so Saturn titles ported across easily and became most of the card's library — but a game built this way could go nowhere else, and the moment Direct3D chose triangles that library stopped growing.", explains: "raster" },
    { year: 1995, name: "QuickDraw 3D RAVE", when: "with QuickDraw 3D 1.0; version 1.1 in 1996", who: "Apple", family: "vendor",
      what: "The Rendering Acceleration Virtual Engine: a vendor-NEUTRAL hardware layer that ATI, NVIDIA and 3dfx all wrote drivers for, and which by 1996 ran on Windows as well as the Mac. The same idea as Direct3D and there before it — but only vendors who cared about the Mac ever implemented it." },
    ...
    { year: 1996, name: "Glide", who: "3dfx", family: "vendor",
      what: "Barely more than Voodoo's register set with names on it. Small, honest and fast, which is why 1996 games looked better on 3dfx than on anything else — and rigid, because the hardware's limits were the API's limits: a 16-bit framebuffer, and textures no larger than 256 by 256.", explains: "texture" },
    { year: 1996, name: "Speedy3D", who: "Rendition", family: "vendor",
      what: "The DOS-side API for the Vérité, whose chip was a programmable RISC core rather than a fixed pipeline: microcode was written per title and downloaded to the card at start-up, and Quake got its own. Programmable graphics hardware, in a consumer slot, in 1996 — on the card fewest people bought." },
    { year: 1996, name: "RRedline", when: "with the V1000 SDK", who: "Rendition", family: "vendor",
      what: "Speedy3D's Windows successor, close enough that the guide told Speedy3D programmers they would recognise it. Rendition-accelerated Tomb Raider ran on it by Christmas." },
    { year: 1996, name: "SGL", who: "VideoLogic and NEC", family: "vendor",
      what: "PowerVR's library. The chips had Direct3D drivers from launch, but an architecture this different — sorting the whole scene into screen tiles before shading any of it — could only be exploited properly through an API built around it." },
    { year: 1996, name: "3D CIF", who: "ATI", family: "vendor",
      what: "ATI's 'C Interface' for the Rage. Like every entry around it, written on the assumption that developers would target each card in turn." },
    { year: 1996, name: "Matrox Simple Interface", who: "Matrox", family: "vendor",
      what: "MSI, for the Mystique — a card with no bilinear filtering, so its API had no way to ask for any." },
    { year: 1996, name: "S3d", who: "S3 Incorporated", family: "vendor",
      what: "The ViRGE's API. S3 would try again with MeTaL two years later, on hardware that deserved it." },
    { year: 1996, name: "Talisman", when: "August, announced", who: "Microsoft", family: "vendor",
      what: "Microsoft's own alternative architecture, shown at SIGGRAPH: composite the frame from cached 2D sub-images instead of redrawing it, to save the bandwidth nobody had. Real accelerators got cheap faster than Talisman got built, and it was cancelled without shipping." },
    ...
    { year: 1997, name: "MiniGL", who: "3dfx", family: "vendor",
      what: "Not an API so much as a concession: a cut-down OpenGL driver implementing just enough of the standard to run GLQuake, written because that is what id Software had chosen to target. Not a matter of principle on id's part — it had shipped VQuake against Rendition's proprietary API seven weeks earlier — but of leverage. The most successful proprietary vendor in the business shipped a rival's interface because one developer could insist.", explains: "pipeline" },
    ...
    { year: 1998, name: "S3 MeTaL", who: "S3 Incorporated", family: "vendor",
      what: "The last proprietary API anyone shipped a hit on. Unreal Tournament's MeTaL renderer loaded S3TC-compressed texture packs that its Direct3D and OpenGL renderers simply would not — the compression was standard by then, but the content was not exposed anywhere else. Visibly better, on one brand of card, a year after that had stopped being a way to sell games.", explains: "texture" },
