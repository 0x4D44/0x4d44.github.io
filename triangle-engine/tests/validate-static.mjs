...
test("the page explains why the vendor APIs lost, structurally rather than by taste", () => {
  ...
  // Unreal shipped with two renderers and accumulated four more in
  // patches; claiming five in the box would be a tidier story and false.
  assert.match(html, /went on sale in 1998 with a software renderer and a Glide one/);
  assert.match(html, /six renderers in the end/);
});
