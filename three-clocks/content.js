...
(function (w) {
  ...
  TC.home = {
    ...
    blocks: [
      ...
      { list: [
        "**Warming lands between about 2.0 and 3.3 degrees by 2100**, with a central estimate near 2.5. The physics has stopped being the uncertain part. What remains uncertain is political and economic, and the range of plausible political outcomes has narrowed sharply — in both directions. 1.5 degrees is gone. So, for different reasons, is the five-degree catastrophe that dominated the 2010s literature: the collapse in the cost of solar generation and storage has quietly removed the worst branch.",
  ...
  TC.method = {
    ...
    blocks: [
      ...
      { p: "The most useful empirical result in the field comes from Philip Tetlock's tournaments, which ran for two decades and scored tens of thousands of dated, resolvable geopolitical forecasts against outcomes. Two findings survive replication. The first is that the average credentialed expert performs badly — in the original study, losing to simple extrapolation algorithms, and losing worst on exactly the long-range questions where expertise is supposed to pay. (The famous line about the dart-throwing chimpanzee is routinely misapplied. It described the worst-performing group, the confident single-theory hedgehogs, on the hardest questions; Tetlock has spent twenty years objecting to the version in which all experts are no better than chance, and the correct claim is both narrower and more useful.) The second finding is that a minority of forecasters, identifiable in advance and consistent across years, beat the base rate substantially." },
      ...
      { list: [
        "**Every world is conditioned on the observed record.** The climate module does not draw a sensitivity and run with it: it draws a sensitivity, computes what warming that implies for 2025, and rejects the combination if it disagrees with the thermometer. Worlds that have already been falsified do not get to be in the cone. This is not free, and the direction is worth stating: about a third of draws are rejected, and the survivors are narrower than the IPCC's published range and slightly hotter — a low-sensitivity world has to explain today's warming with an implausibly large non-CO2 contribution, and a high-sensitivity one with implausibly large aerosol cooling. The model therefore runs a little warm against an unconditioned prior, by roughly three hundredths of a degree per trillion tonnes.",
  ...
  TC.ai = {
    ...
    blocks: [
      ...
      { p: "There is an obvious objection here and it deserves answering rather than ducking: a third of the workforce picking a technology up in three years is *fast*, and it looks like a refutation of any thesis with the word decades in it. It is not, and the reason is the most useful thing the electrification story has to teach. Factories did not take forty years to buy electric motors. They bought them quickly, and bolted them where the steam engine\u2019s driveshaft had been — one big motor turning the same overhead line-shaft, which captured almost none of the benefit. The forty years went on discovering that the point of electricity was that you could put a small motor in every machine and lay the factory out by workflow instead of by proximity to the shaft, and then on rebuilding the factories. **Adoption of the tool is fast; reorganisation around the tool is slow.** Thirty-six per cent of workers using a chat window to do their existing job faster is the drop-in motor. The reorganisation has barely started." },
  ...
  TC.peace = {
    ...
    blocks: [
      ...
      { p: "The data do not support complacency in any case. The Uppsala Conflict Data Programme recorded 65 state-involved conflicts in 2025 — the highest number since the series began in 1946. Thirteen passed the thousand-deaths threshold that defines a war, the most since 1992. Around 244,600 people died in organised violence, the second-worst year since the Rwandan genocide. That total spans three categories UCDP counts separately, and the distinction matters for what follows: state-based armed conflict, non-state conflict, and one-sided violence against civilians. Russia's war in Ukraine alone accounted for about 94,700 deaths — the majority of all battle deaths in state-based conflict, though a smaller share of the 244,600. One-sided violence against civilians rose steeply to 76,500, the highest figure since 1994." },
  ...
  TC.scenarios = {
    ...
    items: [
      ...
      },
      {
        ...
        tell: "It is worth sitting with the fact that this is the highest-weighted bad branch here, that it has the strongest historical base rate of anything on this page, and that it receives a small fraction of the attention given to the others.",
      },
      {
  ...
  TC.forecasts = [
    ...
    { domain: "ai", by: 2030, p: 12,
      ...
      why: "The model puts this nearer 5%, and the gap is deliberate: its misuse channel scales with frontier capability, and I think capability-plus-access outruns that formulation in the near term. The shape matters more than the level — this is a risk that is small now, reaches roughly a fifth by 2040 and four fifths by 2100, and is therefore the one item here whose neglect compounds fastest." },
    ...
    { domain: "ai", by: 2040, p: 45,
      claim: "Between 12% and 40% of employment-weighted work hours in advanced economies are performed without direct human involvement.",
      res: "A task-level accounting on the O*NET basis, of the kind now used in exposure studies.",
      why: "Only 45% for a band that wide, because the model's 2040 distribution is not a smooth hump but close to bimodal: either scaling kept paying and diffusion had begun to bite, or it stalled and the figure is still low single digits. Most of the missing mass sits BELOW this band, not above it. A forecast that hides bimodality behind a central estimate is worse than no forecast." },
    ...
    { domain: "peace", by: 2100, p: 55,
      ...
      why: "The contrarian call, and the one most worth arguing with. It follows from tail arithmetic: conflict has a demonstrated capacity for tens of millions of deaths over a few years, and climate's damage — while larger in total — arrives as a diffuse increase in mortality rather than as an event." },
  ...
  TC.cone = {
    ...
    reading: [
      ...
      { h: "Two things the model does to the picture", p: "Both are defensible and neither is invisible once you know to look. **The bands are smoothed.** Each year's percentile is estimated from a few hundred samples and carries sampling error of its own, so the raw band edge is ragged in a way that invites you to read structure into noise; a five-point moving average removes it without moving the level. **The AI cone's lower edge is a floor, not a forecast.** Deployed automation is modelled as a ratchet — it does not un-deploy when capability stalls — so in the worlds where scaling breaks early the share simply stops where it is. That is why the fifth percentile runs flat at today\u2019s level rather than declining: not a prediction that nothing changes, but a refusal to draw futures in which work that has already been automated is manually re-staffed." },
      { h: "The coupling switch", p: "Turn the cross-domain couplings off and, at the default settings, 2100 warming moves by about two hundredths of a degree, the nuclear hazard by three or four percentage points, and median conflict deaths by around a sixth. Those are not nothing — the conflict figure in particular is a real effect — but they are an order of magnitude smaller than the weight the couplings carry in public argument. These three problems interact; they are not one problem, and the interactions are mostly not what carries the risk." },
    ],
  };
    ...
    items: [
      {
        ...
        resp: "Largely conceded. The model responds to the first half by making the deceleration structural rather than optional, and by carrying a per-year hazard of a regime break that produces a genuine plateau. On the diffusion analogy there is a real answer, given in the AI section: the electrification lag was never about how fast factories bought motors, it was about how long they took to stop bolting them where the driveshaft had been, and adoption-versus-reorganisation is a distinction the fast-adoption statistics do not settle either way. But that answer defends the analogy; it does not establish it. The general-purpose-technology analogy is load-bearing and it is an analogy. If AI is better modelled as a new class of agent rather than as a new class of tool, the diffusion lag is the wrong concept and the timelines here are too slow by a wide margin.",
      },
      {
        h: "The nuclear probability is alarmism dressed as arithmetic",
        arg: "The figure — a little over two in five, at the default settings — comes from compounding a small annual hazard across seventy-five years, which is a mathematical operation, not a forecast. Eighty-one years of non-use is strong evidence that the process is not what the model says it is — that deterrence is a stable equilibrium rather than a lottery, and that the states holding these weapons behave far more carefully than a constant hazard rate implies.",
        resp: "The strongest objection on this page, and it may be right. Two things in reply. Compounding a small hazard is exactly how one ought to reason about rare events over long horizons — the alternative, which is to note that it has not happened yet, is how people reason about rare events immediately before they happen. And the eighty-one-year record cannot distinguish a stable equilibrium from a quiet draw, which is the Cirillo–Taleb point applied to the nuclear case. There is also a comparison worth making before reaching for the word alarmism: this model's peacetime hazard sits below the published central estimates, and its century figure is lower than naively compounding them. That said, the figure is highly sensitive to the wartime multiplier, which is a judgement rather than a measurement, and moving the sliders on the cone page moves it a great deal. It should be read as 'this is what these assumptions imply', not as a claim to know.",
      },
      {
