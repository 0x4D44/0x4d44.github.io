...
const mustExist = ['index.html','styles.css','content.js','engine.js','storage.js','app.js','manifest.webmanifest','sw.js','icons/icon.svg'];
...
assert.match(index, /<link rel=\x22manifest\x22 href=\x22manifest\.webmanifest\x22\s*\/>/, 'index links manifest');
...
for (const file of mustExist.filter((file) => file !== 'sw.js')) assert.ok(sw.includes(`\x22${file}\x22`) || (file === 'index.html' && sw.includes('\x22./\x22')), `service worker precaches ${file}`);
assert.match(sw, /startsWith\(CACHE_PREFIX\)/, 'service worker cleans only this app cache family');
...
for (const file of ['content.js','engine.js','storage.js','app.js','sw.js']) execFileSync(process.execPath, ['--check', join(appDir, file)], { stdio: 'pipe' });
...
for (const exercise of EXERCISES) {
  for (const field of ['id','name','category','purpose','longevityWhy','beginnerVersion','progression','loggingType','diagramType']) assert.ok(exercise[field], `${exercise.id} has ${field}`);
  for (const field of ['equipment','locationModes','instructions','commonMistakes','safetyNotes']) assert.ok(Array.isArray(exercise[field]) && exercise[field].length, `${exercise.id} has ${field}`);
}
...
assert.match(app, /navigator\.serviceWorker\.register\(\x22\.\/sw\.js\x22\)/, 'app registers service worker');
...
assert.ok(existsSync(root), 'repository root exists for GitHub Pages route');
